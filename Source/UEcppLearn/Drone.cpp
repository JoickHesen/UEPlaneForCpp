...
void ADrone::Tick(float DeltaTime)
{
	...
	/*float LiftValue = InputComponent->GetAxisValue(TEXT("Lift"));
	float ForwardValue = InputComponent->GetAxisValue(TEXT("Forward"));
	UE_LOG(LogTemp, Warning, TEXT("Lift: %f"), LiftValue);*/
...
void ADrone::Forward(float val){
	...
	if (FMath::Abs(pitch) < 30.0f) {
		Mesh->AddRelativeRotation(FRotator(-  100.0 * GetWorld()->DeltaTimeSeconds * val, 0.0f, 0.0f));
	}
}
...
void ADrone::RoatePaddle(float Detal) {
	for (auto paddle : Paddles) {
		paddle->AddRelativeRotation(FRotator(0.0f, Detal * PaddleRoateSpeed, 0.0f));
	}
}
